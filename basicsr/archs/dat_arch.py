...
from basicsr.archs.fusion_blocks import FusionBlock
...
class RealDAT(DAT):
    def __init__(self,
                ...
                **kwargs):
        super().__init__(img_size, in_chans, embed_dim, split_size, depth, num_heads, expansion_factor, qkv_bias, qk_scale, drop_rate, attn_drop_rate, drop_path_rate, act_layer, norm_layer, use_chk, upscale, img_range, resi_connection, upsampler, **kwargs)

        if fusion_layers == 'all':
            fusion_layers = set(range(self.num_layers))
        elif fusion_layers == 'none':
            fusion_layers = set([])
        elif isinstance(fusion_layers, int):
            fusion_layers = {fusion_layers}
        else:
            fusion_layers = set([int(l) for l in fusion_layers.split('-')]) # e.g., '0-2-4'

        self.fusion_blocks = nn.ModuleList([
			None if i not in fusion_layers else FusionBlock(
				dim=embed_dim, num_heads=fusion_num_heads,attn_ratio=fusion_attn_ratio,
                mlp_ratio=fusion_mlp_ratio, qkv_bias=True,drop=fusion_drop_rate,
                attn_drop=fusion_attn_drop_rate, drop_path=fusion_drop_path_rate)
			for i in range(self.num_layers)])

        self.adaptive_feat_channels = nn.Sequential(
    ...
    def forward_features(self, x, xi=None, xd=None):
        _, _, H, W = x.shape
        x_size = [H, W]
        x = self.before_RG(x) # b (h w) c
        for fusion_block, layer in zip(self.fusion_blocks, self.layers):
            if fusion_block and (xi is not None) and (xd is not None):
                x, xi, xd = fusion_block(x, xi, xd)
            x = layer(x, x_size)
        x = self.norm(x)
        x = rearrange(x, "b (h w) c -> b c h w", h=H, w=W).contiguous()
    ...
    def forward(self, x, xi=None, xd=None):
        """
        Input: x: (B, C, H, W) C=3
        xi: (B, ni, C1) ni=1 C1=512
        xd: (B, nd, C1) nd=5 C1=512
        """
        self.mean = self.mean.type_as(x)
        x = (x - self.mean) * self.img_range
        xi = self.adaptive_feat_channels(xi)
        xd = self.adaptive_feat_channels(xd)

        if self.upsampler == 'pixelshuffle':
            # for image SR
            x = self.conv_first(x)
            x = self.conv_after_body(self.forward_features(x, xi, xd)) + x
            x = self.conv_before_upsample(x)
            x = self.conv_last(self.upsample(x))
        elif self.upsampler == 'pixelshuffledirect':
            # for lightweight SR
