...
from .srclip_model import SrCLIP, SrCLIPV2, FinetuneCLIP
from .loss import (
    ClipLoss,
    DistillClipLoss,
    CoCaLoss,
...
def load_checkpoint(
    ...
    if "logit_bias" not in state_dict and hasattr(model, "logit_bias"):
        if model.logit_bias:
    ...
    **model_kwargs,
    ...
    if pretrained and pretrained.lower() == "openai":
        ...
        if custom_text:
            ...
            elif "srclip_" in model_name:
                clip_model = CLIP(**model_cfg, cast_dtype=cast_dtype)
                model = SrCLIP(clip_model)
            elif "srclipv2_" in model_name:
                clip_model = CLIP(**model_cfg, cast_dtype=cast_dtype)
                model = SrCLIPV2(
                    clip_model, scale_degra_dim=5
                )  # FIXME: hardcoded scale_degra_dim
            elif "FinetuneCLIP_" in model_name:
                model = FinetuneCLIP(**model_cfg, scale_degra_dim=5)  # FIXME: hardcoded scale_degra_dim
            else:
        ...
        if pretrained:
            checkpoint_path = ""
            if "srclip_" in model_name:
                pretrained_cfg = get_pretrained_cfg(model_name[7:], pretrained)
            elif "srclipv2_" in model_name:
                pretrained_cfg = get_pretrained_cfg(model_name[9:], pretrained)
            elif "FinetuneCLIP_"in model_name:
                pretrained_cfg = get_pretrained_cfg(model_name[13:], pretrained)
            else:
            ...
            if checkpoint_path:
                if "srclip_" in model_name and is_train:
                    logging.info(f"Loading pretrained {model_name[7:]} weights ({pretrained}).")
                    load_checkpoint(model.clip, checkpoint_path)
                    model.initial_controller()
                    model.lock_clip()
                elif "srclipv2_" in model_name and is_train:
                    logging.info(f"Loading pretrained {model_name[9:]} weights ({pretrained}).")
                    load_checkpoint(model.clip, checkpoint_path)
                    model.initial_controller()
                    model.lock_clip()
                elif "FinetuneCLIP_" in model_name and is_train:
                    logging.info(f"Loading pretrained {model_name[13:]} weights ({pretrained}). Strict=False")
                    load_checkpoint(model, checkpoint_path, strict=False)
                elif not is_train:
                    logging.info(f"Loading pretrained {model_name} weights ({pretrained}) to eval. Strict=False")
                    load_checkpoint(model, checkpoint_path, strict=False)
                else:
...
def create_loss(args):
    if args.distill:
    ...
    elif "srclip_" in args.model and not args.siglip:
        return SrClipLoss(
            local_loss=args.local_loss,
            gather_with_grad=args.gather_with_grad,
            cache_labels=True,
    ...
    elif "srclipv2_" in args.model or "FinetuneCLIP_" in args.model: #TODO no hardcode
        return SrClipLossV2(
            local_loss=args.local_loss,
            gather_with_grad=args.gather_with_grad,
            cache_labels=True,
        ...
        ) if not args.siglip else SrClipSigLipLoss(
            rank=args.rank,
            world_size=args.world_size,
            weights=args.loss_weight,
        )
